...
class AddLooksItemPage extends StatefulWidget {
  static const String name = "add-looks-item";

  const AddLooksItemPage({super.key});

  @override
  State<AddLooksItemPage> createState() => _AddLooksItemPageState();
}

class _AddLooksItemPageState extends State<AddLooksItemPage> {
    ...
    if (!_isInitialized) {
      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
        ...
        if (extra != null) {
          ...
          if (imagePath != null) {
            context.read<LooksListProvider>().setNewItemImagePath(imagePath);
          }
        }
        _isInitialized = true;
      });
  ...
  Future<void> _saveClothingItem() async {
    ...
    try {
      ...
      if (mounted) {
        context.go('/${CreateStylePage.name}');
      }
    } finally {
            ...
            children: [
              if (provider.newItemImagePath == null)
                const AddPhotoSection(
                  returnTo: AddLooksItemPage.name,
                ),
              if (provider.newItemImagePath != null)
                Container(
                  width: 400,
